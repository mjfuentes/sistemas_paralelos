    ...
    while(ok == 1)
    {
        if(rank != 0){ // Cada uno de los workers
            ...
            if (tag < size){
                ...
                merge(lcl,tmp,0, N/size - 1, (N/size * 2) - 1);
        ...
        else // master
        {
            ...
            if (tag == -1){
                ...
                else if (step < (size + size/2)){
                    MPI_Isend(&(A[step*(N/size*2)]),N/size*2,MPI_INT,status.MPI_SOURCE,step,MPI_COMM_WORLD,&request);
                    step = step + 1;
                }
                else {
                    MPI_Isend(&(lcl[0]),0,MPI_INT,status.MPI_SOURCE,step,MPI_COMM_WORLD,&request);
                    out++;
                    if (out >= size){
                        ok = 0;
                    }
